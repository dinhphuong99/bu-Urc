...
{
    [Area("Admin")]
    public class UrencoCostTypeController : BaseController
    {
        private readonly EIMDBContext _context;
        private readonly IStringLocalizer<UrencoCostTypeController> _stringLocalizer;
        private readonly IStringLocalizer<SharedResources> _sharedResources;

        public UrencoCostTypeController(EIMDBContext context, IStringLocalizer<UrencoCostTypeController> stringLocalizer, IStringLocalizer<SharedResources> sharedResources)
        {
            _context = context;
        ...
        public object JTable([FromBody]JTableModelMaterial jTablePara)
        {
            ...
            var query = from a in _context.UrencoCostTypes
        ...
        [HttpPost]
        public object Insert([FromBody]UrencoCostType data)
        {
            var msg = new JMessage() { Error = false };
            try
            {
                if (_context.UrencoCostTypes.FirstOrDefault(x => x.Code == data.Code && !x.IsDeleted) == null)
                {
                    data.CreatedBy = ESEIM.AppContext.UserName;
                    data.CreatedTime = DateTime.Now;

                    _context.UrencoCostTypes.Add(data);
                ...
                {
                    msg.Error = true;
                    msg.Title = _stringLocalizer["UCT_MSG_CODE_EXIST"];
                }
            }
            catch (Exception ex)
            {
                msg.Error = true;
                msg.Title = String.Format(_sharedResources["COM_MSG_ADD_FAILED"], _stringLocalizer["UCT_LBL_UCT"]);
                msg.Object = ex;
            }
            return Json(msg);
        }
        ...
        public object Delete(int id)
        {
            ...
            try
            {
                var data = _context.UrencoCostTypes.FirstOrDefault(x => x.Id == id);
                data.IsDeleted = true;
                data.DeletedBy = ESEIM.AppContext.UserName;
                data.DeletedTime = DateTime.Now;

                _context.UrencoCostTypes.Update(data);
                _context.SaveChanges();
        ...
        [HttpPost]
        public object Update([FromBody]UrencoCostType data)
        {
            var msg = new JMessage() { Error = false };
            try
            {
        ...
        {
            var data = _context.UrencoCostTypes.Select(x => new { x.Id, x.Name }).AsNoTracking().ToList();
            return Json(data);
        }
