...
namespace III.Admin.Controllers
{
    ...
    public class AssetRPTBrokenController : BaseController
    {
        ...
        public AssetRPTBrokenController(EIMDBContext context, IUploadService upload, IHostingEnvironment hostingEnvironment, IStringLocalizer<ContractController> contractController, IStringLocalizer<AssetInventoryController> inventoryController,
            IStringLocalizer<CustomerController> stringLocalizerCus, IStringLocalizer<FileObjectShareController> stringLocalizerFile, IStringLocalizer<AssetRPTBrokenController> stringLocalizer, IStringLocalizer<SharedResources> sharedResources)
        {
            _context = context;
            _upload = upload;
        ...
        public object JTable([FromBody]JTableModelAct jTablePara)
        {
            ...
            var query = (from a in _context.AssetRPTBrokenDetails
        ...
        public object JTableTicket([FromBody]JTableModelAct jTablePara)
        {
            ...
            var listCommon = _context.CommonSettings.Select(x => new { x.CodeSet, x.ValueSet });
            var query = (from a in _context.AssetRPTBrokenHeaders
                         join b in _context.AdOrganizations on a.Branch equals b.OrgId.ToString() into b1
                         from b2 in b1.DefaultIfEmpty()
                         join c in _context.HREmployees on a.Person equals c.Id.ToString() into c1
                         from c2 in c1.DefaultIfEmpty()
                         join d in _context.CommonSettings.Where(x => x.Group == "ASSET_URENCO") on a.AssetStatus equals d.CodeSet into d1
                         from d2 in d1.DefaultIfEmpty()
                         join e in _context.AdOrganizations on a.Branch equals e.OrgCode into e1
                         from e2 in e1.DefaultIfEmpty()
                         where (!a.IsDeleted 
        ...
        public JsonResult Insert([FromBody]AssetRPTBrokenHeadersJtableModel obj)
        {
            ...
            try
            {
                ...
                if (data == null)
                {
                    ...
                    dt.AssetStatus = obj.AssetStatus;
                    //Insert auto log
