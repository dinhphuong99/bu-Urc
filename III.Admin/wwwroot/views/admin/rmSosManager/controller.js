var ctxfolder = "/views/admin/rmSosManager";
...
app.controller('index', function ($scope, $rootScope, $compile, $confirm, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter) {
   ...
   // vm.dtColumns.push(DTColumnBuilder.newColumn("Id").withTitle(titleHtml).notSortable()
    ...
    //    return data;
    //}));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Code').withTitle('Mã (SOS Code)').withOption('sClass', 'sorting_disabled').renderWith(function (data, type) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Data').withTitle('Vị trí location').renderWith(function (data, type, full) {
        if (data == 'null' || data == 'undefined')
            return '';
        else
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Address').withTitle('Địa chỉ').renderWith(function (data, type, full) {
        return data;
        //if (data == 'null' || data == 'undefined')
        //    return '';
        //else
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('Path').withTitle('Ảnh').renderWith(function (data, type) {

        var dt = '<img style="width:64px; height:64px" src="' + data + '" onerror =' + "'" + 'this.src="' + '../../../images/default/no_image.png' + '"' + "'" + ' class="img-responsive">';
        //return '<img style="width:64px; height:64px" src="' + data + ' class="img-responsive" > ';
        return dt;
    }));
...
app.controller('google-map', function ($scope, $rootScope, $compile, $uibModal, $confirm, $uibModalInstance, DTColumnBuilder, DTInstances, dataservice, $filter, para) {
    ...
    setTimeout(function () {
        fields_vector_source = new ol.source.Vector({});
        var center = ol.proj.transform(para, 'EPSG:4326', 'EPSG:3857');
        map = new ol.Map({
            target: $('#map')[0],
        ...
        var iconFeatureStart = new ol.Feature({
            geometry: new ol.geom.Point(ol.proj.transform(para, 'EPSG:4326', 'EPSG:3857')),
            type: "valve"
        });
        var styles3 = [
