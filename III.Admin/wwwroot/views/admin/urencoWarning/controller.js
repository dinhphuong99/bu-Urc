var ctxfolder = "/views/admin/UrencoWarning";
...
app.controller('Ctrl_ESEIM', function ($scope, $rootScope, $cookies, $translate, dataservice, $filter) {
    ...
    dataservice.getStatusPOSup(function (rs) {rs=rs.data;
        $rootScope.status = rs;
    });
...
app.config(function ($routeProvider, $validatorProvider, $translateProvider, $httpProvider) {
    $translateProvider.useUrlLoader('/Admin/UrencoWarning/Translation');
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $window, $filter) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/UrencoWarning/JTableWarning",
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('WarningType').withTitle('Loại cảnh báo').renderWith(function (data, type) {
        return data;
    }));
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('GPS').withTitle('Điểm GPS').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('GPSText').withTitle('Địa điểm').renderWith(function (data, type) {
        return data;
    }));
...
app.controller('add', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, DTColumnBuilder, DTInstances, dataservice, $filter) {
    ...
    function initData() {
        ...
        dataservice.genPoSupCode(function (result) {result=result.data;
    ...
    $scope.openLog = function () {
        dataservice.getUpdateLog($scope.model.PoSupCode, function (rs) {rs=rs.data;
            var modalInstance = $uibModal.open({
                animation: true,
...
app.controller('addProduct', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter) {
    ...
    $scope.add = function () {
        if ($rootScope.PoSupCode != '') {
            ...
            if ($scope.addform.validate() && !validationSelect($scope.model).Status) {
                dataservice.insertDetail($scope.model, function (rs) {rs=rs.data;
                    if (rs.Error) {
                        App.toastrError(rs.Title);
                    }
                    else {
    ...
    $scope.save = function (id) {
        validationSelect($scope.model);
        if ($scope.addform.validate() && !validationSelect($scope.model).Status) {
            dataservice.updateDetail($scope.model, function (rs) {rs=rs.data;
                if (rs.Error) {
                    App.toastrError(rs.Title);
                }
                else {
    ...
    $scope.delete = function (id) {
        var modalInstance = $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.ok = function () {
                    dataservice.deleteDetail(id, function (result) {result=result.data;
                        if (result.Error) {
                            App.toastrError(result.Title);
                        } else {
                            App.toastrSuccess(result.Title);
...
app.controller('activity', function ($scope, $rootScope, $compile, $uibModal, $location, $filter, $uibModalInstance, dataservice, $timeout, para) {
    ...
    $scope.add = function () {
        if ($scope.model.Confirm != '' && !$scope.isEdit) {
            dataservice.insertConfirmText(para, $scope.model.Confirm, function (rs) {rs=rs.data;
                if (rs.Error) {
                    App.toastrError(rs.Title);
                } else {
                    App.toastrSuccess(rs.Title);
...
app.controller('attributeManager', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, $location, $filter, $uibModalInstance, dataservice, $timeout, para) {
    ...
    $scope.add = function () {
        ...
        dataservice.insertContractAttr($scope.model, function (rs) {rs=rs.data;
            if (rs.Error) {
                App.toastrError(rs.Title);
            }
            else {
    ...
    $scope.delete = function (id) {
        var modalInstance = $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                $scope.message = caption.COM_MSG_DELETE_CONFIRM_COM;//"Bạn có chắc chắn muốn xóa?";
                $scope.ok = function () {
                    dataservice.deleteContractAttr(id, function (result) {result=result.data;
                        if (result.Error) {
                            App.toastrError(result.Title);
                        } else {
                            App.toastrSuccess(result.Title);
    ...
    $scope.save = function (id) {
        ...
        dataservice.updateContractAttr($scope.model, function (rs) {rs=rs.data;
            if (rs.Error) {
                App.toastrError(rs.Title);
            }
            else {
