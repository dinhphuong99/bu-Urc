...
// var host = "http://tamlong.s-work.vn";
...
var pins = [
    ...
    { name: "Vicem", pin: "VC" },
    { name: "S-work", pin: "SW" },
	{ name: "Phòng Test", pin: "PT" },
	{ name: "Hà Nội, Việt Nam", pin: "HN.VN" }	
]
var currentRoom = pins[4].pin;
var list = [10000];
var data;
var map, polygon, id;
id = 0;
...
var drawMarkerExist = function (data) {
    ...
    var book = data.locationMessage;
    var id = book.driverId;
    console.log(" -------------> ");
    console.log(data);
    console.log(" <---------------------------------------------------------------------------------------------------------");
    if (data != null) {
        if (carSourceVector.getFeatureById(id) != null) {
            ...
            var lon = coord[0];
            var lat = coord[1];
            if (Contains(book.longitude, book.latitude, polygon)) {
                var bear = bearing(lat, lon, book.latitude, book.longitude);
                var lonlat3857 = new ol.geom.Point(ol.proj.transform([book.longitude, book.latitude], 'EPSG:4326',
                    'EPSG:3857'));
...
function styleFunction() {
    ...
    if (zoom >= 8) {
        if (zoom >= 13) {
            ...
            html = "<b " + "style='font-size:1em'" + ">" + "Lái xe: " + this.get("name") + "</b>";
        }
        map.addOverlay(popup);
    ...
    //var html = "Lái xe: " + data.locationMessage.driverName;
    ...
    return [
        ...
        new ol.style.Style({
            image: new ol.style.Icon(({
                ...
                src: 'https://www.upsieutoc.com/images/2019/03/26/car.png'
            })),
        })
    ];
}
